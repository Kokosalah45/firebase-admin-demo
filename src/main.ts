...
const userDetailTemplate = (userDetail: UserDetail) => {
  ...
  return `
  <div data-user-details=${JSON.stringify({
    newUserAge,
    newUserName,
    newUserOccupation,
    id,
...
const userListBtn = document.getElementById(
  "user-list-btn"
...
const newUserOccupation = document.querySelector(
  'input[name="newUserOccupation"]'
) as HTMLInputElement;
const newUserAge = document.querySelector(
  'input[name="newUserAge"]'
) as HTMLInputElement;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  addUserForm.addEventListener("submit", async (e) => {
    ...
    await addDoc(usersCollection, {
      ...
      newUserOccupation: newUserOccupation.value,
      newUserAge: newUserAge.value,
    });
  });
  ...
  userListBtn.addEventListener("click", async () => {
    try {
      ...
      userResultContainer!.innerHTML = "";
      docSnap.forEach((doc) => {
        const userDetailTemp = userDetailTemplate({
          id: doc.id,
          ...doc.data(),
        });
  ...
  userResultContainer?.addEventListener("click", (e) => {
    ...
    if (t.classList.contains("delete")) {
      const userDetails = JSON.parse(
        t.parentElement?.parentElement?.dataset.userDetails || ""
      ) as UserDetail;
